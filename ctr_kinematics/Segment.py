...
class Segment:
    ...
    def __init__(self, t1, t2, t3=None, base=None):
        if t3 is not None:
            ...
            num_tubes = 2
            index_iterator = [1, 3, 5]
            stiffness = np.array([t1.E, t2.E])
            torsion = np.array([t1.G, t2.G])
            curve_x = np.array([t1.U_x, t2.U_x])
            curve_y = np.array([t1.U_y, t2.U_y])

            d_tip = np.array([t1.L, t2.L]) + base  # position of tip of the tubes
            d_c = d_tip - np.array([t1.L_c, t2.L_c])  # position of the point where tube bending starts

        points = np.hstack((0, base, d_c, d_tip))
        index = np.argsort(points)
        segment_length = 1e-5 * np.floor(1e5 * np.diff(np.sort(points)))

        e = np.zeros((num_tubes, segment_length.size))
        g = np.zeros((num_tubes, segment_length.size))
        u_x = np.zeros((num_tubes, segment_length.size))
        u_y = np.zeros((num_tubes, segment_length.size))

        for i in range(0, num_tubes):
            aa = np.where(index == i + index_iterator[0])  # Find where tube begins
            a = aa[0]
            bb = np.where(index == i + index_iterator[1])  # Find where tube curve starts
            b = bb[0]
            cc = np.where(index == i + index_iterator[2])  # Find where tube ends
            c = cc[0]
        ...
        e_t = ee[length_sum + min(base) > 0 * ee].reshape(num_tubes, len(self.S))
        g_t = gg[length_sum + min(base) > 0 * ee].reshape(num_tubes, len(self.S))
        if num_tubes == 3:
            self.EI = (e_t.T * np.array([t1.I, t2.I, t3.I])).T
            self.GJ = (g_t.T * np.array([t1.J, t2.J, t3.J])).T
        else:
            self.EI = (e_t.T * np.array([t1.I, t2.I])).T
            self.GJ = (g_t.T * np.array([t1.J, t2.J])).T
        self.U_x = uu_x[length_sum + min(base) > 0 * ee].reshape(num_tubes, len(self.S))
        self.U_y = uu_y[length_sum + min(base) > 0 * ee].reshape(num_tubes, len(self.S))
