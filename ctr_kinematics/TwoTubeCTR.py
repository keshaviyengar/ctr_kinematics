...
class TwoTubeCTRKinematics(object):
    ...
    def forward_kinematics(self, joints):
        ...
        :param joints: Current joint position of robot [beta_0, ..., beta_2, alpha_0, ..., alpha_2]
        :return: End effector position or achieved goal of selected system.
        """
        beta = joints[:2]
        segment = Segment(self.system_parameters[0], self.system_parameters[1], self.system_parameters[2], beta)
    ...
    def kinematic_model(self, uz_0, alpha_0, r_0, R_0, segmentation, beta):
        ...
        for seg in range(0, len(segmentation.S)):
            ...
            u_z = np.vstack((u_z, s[:, (2, 3)]))
            alpha = np.vstack((alpha, s[:, (4, 5)]))
...
# TODO: Sort out ode for two tube system.
